...
' Page Data 
' =======================
Sql = "SELECT  top 15 * FROM  booklist  WHERE (disctype = 'C') AND ([language] = 1) AND (distribute = 2) AND (disctype1 = 1)  and  stflg <> 2 and (sb_oh+sb14_oh) > 5 order by CONVERT(datetime,recvdate,5) desc"
Set RS = Server.CreateObject("ADODB.RecordSet")
RS.Open Sql,conn,1,3
' =======================
%>
<font class="text_header">หนังสือออกใหม่</font>
	<%Do While not rs.EOF%>
    <table width="100%" border="0" cellspacing="2" cellpadding="2">      
	  ...
	  %>
        ...
        <td width="93%" valign="top"><div align="left"><a href="description.asp?barcode=<%=RS("barcode")%>" class="text_normal"><b><%=RS("title")%><%=RS("title1")%></b></a><br />
          <%=RS("barcode")%><br />
