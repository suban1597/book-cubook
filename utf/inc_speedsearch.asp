...
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh  FROM  booklist WHERE  "
...
'sql = sql & " and language=1 and (sb14_oh+stock_oh+jj_oh+cb_oh)>=0 and([year] BETWEEN '"&tDate&"' and  '"&tDate2&"') or (language=1 and (sb14_oh+stock_oh+jj_oh+cb_oh)>=2)"
sql = sql & " and language=1 and([year] BETWEEN '"&tDate&"' and  '"&tDate2&"') or (language=1)"		
...
'sql = sql & " and language=2 and (sb14_oh+stock_oh+jj_oh+cb_oh)>=0 and([year] BETWEEN '"&eDate&"' and  '"&eDate2&"') or (language=2 and (sb14_oh+stock_oh+jj_oh+cb_oh)>=1)"		
sql = sql & " and language=2 and([year] BETWEEN '"&eDate&"' and  '"&eDate2&"') or (language=2)"	
sql=sql&"order by CONVERT(datetime,recvdate,5) DESC"
End If
' ============================================
%>
...
' ============================================
If Option1="VDO" Then
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh FROM  booklist WHERE booklist.category between '8100' and '8174' and "
		For k = 1 to nkeyword
			If Not (k = nkeyword) Then	
				sql=sql &" (title+title1 like '%"& replace(pKeyword(k),"'","''") & "%') AND "
			Else	
...
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh  FROM  booklist WHERE booklist.category between '8300' and '8374' and "
		For k = 1 to nkeyword
				If Not (k = nkeyword) Then		
						sql=sql &" (title+title1 like '%"& replace(pKeyword(k),"'","''") & "%') AND "
				Else	
...
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh  FROM  booklist WHERE booklist.category between '8400' and '8474' and "
		For k = 1 to nkeyword
				If Not (k = nkeyword) Then				
						sql=sql &" (title+title1 like '%"& replace(pKeyword(k),"'","''") & "%') AND "
				Else		
...
' ===========================================
If Option1="CD" Then
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh  FROM  booklist WHERE booklist.category between '8500' and '8574' and "
		For k = 1 to nkeyword
				If Not (k = nkeyword) Then
						sql=sql &" (title+title1 like '%"& replace(pKeyword(k),"'","''") & "%') AND "
				Else
...
' ===========================================
If Option1="VCD" Then
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh FROM  booklist  WHERE booklist.category between '8600' and '8674' and "
		For k = 1 to nkeyword
				If Not (k = nkeyword) Then
						sql=sql &" (title+title1 like '%"& replace(pKeyword(k),"'","''") & "%') AND "
				Else  
...
' ============================================
If Option1="DVD" Then
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh FROM  booklist WHERE booklist.category between '8800' and '8874' and "
		For k = 1 to nkeyword
				If Not (k = nkeyword) Then
						sql=sql &" (title+title1 like '%"& replace(pKeyword(k),"'","''") & "%') AND "
				Else
...
sql="Select  barcode,title+title1 as title,author,edition,[year],isbn,price,disctype,disctype1,language,distribute,translator,sb14_oh,stock_oh,jj_oh,cb_oh FROM  booklist WHERE "
		For k = 1 to nkeyword
				...
				End If
		Next
sql=sql & " and ((language=1) or (language=2 and ([year] BETWEEN '"&eDate&"' and  '"&eDate2&"' )) or (language=2) or (language=3 and ([year] BETWEEN '"&tDate&"' and  '"&tDate2&"' )))" '08-07-2016
...
sql=sql&"order by sb14_oh+stock_oh+jj_oh+cb_oh DESC, [year] DESC"
End If
' ============================================
%>
...
sql=sql & " and ((language=1) or (language=2 and ([year] BETWEEN '"&eDate&"' and  '"&eDate2&"' )) or (language=2) or (language=3 and ([year] BETWEEN '"&tDate&"' and  '"&tDate2&"' ) OR booktype = 2 ))" '28-09-2016
'sql=sql & " and ((language=1 and (sb_oh+sb14_oh+jj_oh)>=0) or (language=2 and (sb_oh+sb14_oh+jj_oh)=0 and ([year] BETWEEN '"&eDate&"' and  '"&eDate2&"' )) or (language=2 and (sb_oh+sb14_oh+jj_oh)>=2) or (language=3 and (sb_oh+sb14_oh+stock_oh+jj_oh)>=5 and ([year] BETWEEN '"&tDate&"' and  '"&tDate2&"' )))" '08-07-2016
...
If RS("booktype") = 1 Then
	If RS("Language") = 1 Then
		total_oh = RS("sb14_oh")+RS("cb_oh")+RS("jj_oh")
		If total_oh >= 2 Then
			...
			'if RS("Barcode") = 9786164454507 OR RS("Barcode") = 9786164454491 THEN
			''	response.write "<a href='http://www.chulabook.com/king/login.php'><img src=images/icons/cart.png border=0/></a> "
			'else
			'response.write "<img src=images/icons/non-cart.png   border=0 name=image2 />"
			response.write "<img src=images/button/outofstock.png   border=0 name=image2 height=25px />"
		...
		total_oh2 = RS("sb14_oh")+RS("jj_oh")+RS("cb_oh")
		If total_oh2 >= 2 Then
			response.write "<input type=image src=images/icons/cart.png alt=Add to cart  title=หยิบใส่ตระกร้า border=0 name=image />"
		...
		total_oh3 = RS("sb14_oh")+RS("jj_oh")+RS("cb_oh")
		If total_oh3 >= 5 Then
			response.write "<input type=image src=images/icons/cart.png alt=Add to cart  title=หยิบใส่ตระกร้า border=0 name=image />"
			'response.write total_oh2
		else if RS("stock_oh") >= 20 Then
