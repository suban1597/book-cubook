<%
sql="SELECT Ebooklist.isbn, Ebooklist.book_name, booklist.author, booklist.price, Ebooklist.book_cover_price, Ebooklist.book_bath_price,Ebooklist.book_id,Ebooklist.book_thumbnail_path,booklist.barcode,booklist.title,booklist.title1 FROM Ebooklist INNER JOIN booklist ON Ebooklist.isbn = booklist.barcode WHERE (Ebooklist.enable = '1') AND (Ebooklist.recommend = 'Y') ORDER BY Ebooklist.create_date DESC"
%>
<%
...
%>
    ...
    <form id="addtocart" name="addtocart" method="post" action="shopping.asp">
        <a href="description_ebook.asp?barcode=<%=RS("barcode")%>" class="blacktext"><b><%=RS("book_name")%></b></a><br>
  ...
  <tr>
    <td><div align="left">
      ...
      <div align="left">
        ...
        <a href="all_ebook_recommend.asp?pagecount=<%=itscount%>"> <%=itscount%></a>
        <%next%>
        <!--<A href="#">¶Ñ´ä» >></A>  -->
        <br />
        <br />
