...
RSOrders.Open " SELECT OrderID,name,address,Province,zip,phone,Orderdate,Ordertime,PaymentMethod,Credit_approve,Amount,SAHC,orderstatus,remark_cancel,UserID FROM orders Where OrderID='"&OrderID&"'", Conn, 1, 3
...
เวลา <%= Mid(RSOrders("Ordertime"),1,2) %>:<%= Mid(RSOrders("Ordertime"),3,2) %>:<%= Mid(RSOrders("Ordertime"),5,2) %> </div></td>
      ...
      <tr>
        <td bgcolor="#FFFFFF"><div align="right">สถานะการชำระเงิน : </div></td>
        <td height="25" bgcolor="#FFFFFF"><div align="left">&nbsp;&nbsp; 
          <%
          sql_checkstatus = "SELECT orders.OrderID AS OrderIDchk , banktransfer.TrackNo AS TrackNochk FROM banktransfer INNER JOIN orders ON banktransfer.TrackNo = orders.OrderID WHERE (banktransfer.TrackNo = '"&Orderid&"')"
          Set RS_checkstatus = Server.CreateObject("ADODB.RecordSet")
          RS_checkstatus.Open Sql_checkstatus,conn,1,3
          
          
          If RSOrders("PaymentMethod") = 2 Then
          
            If  RSOrders("Credit_approve")=1 Then
              response.Write "สำเร็จ"
            'else isNull(RSOrders("Credit_approve"))
            else 
              response.Write "ไม่สำเร็จ"
            end if
          elseif RSOrders("PaymentMethod") = 7 Then
            If  RSOrders("Credit_approve")=1 Then
              response.Write "สำเร็จ"
            else 
              response.Write "ไม่สำเร็จ"
            end if
          elseif RSOrders("PaymentMethod") = 1 Then
            response.Write "เก็บเงินสดปลายทาง"
          elseif Not RS_checkstatus.EOF Then
            response.Write "แจ้งผลการโอนเงินแล้ว"
          elseif RS_checkstatus.EOF Then
            response.Write "ยังไม่แจ้งผลการโอนเงิน"
          else 
            response.Write "-"
          end if
          	     ...
          	     RS_Post.Open " SELECT orderid FROM PostInformation WHERE orderid = '"&RSOrders("orderid")&"' ", Conn, 1, 3
            	     If Not RS_Post.EOF Then
            	       response.Write "ส่งสินค้าเรียบร้อยแล้ว"
                        ...
                        RS_Kerry.Open " SELECT order_no FROM KerryExpress WHERE order_no = '"&RSOrders("orderid")&"' ", Conn, 1, 3
...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#CCCCCC"><table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" class="blacktext">
      <tr>
        <td height="25" colspan="5" bgcolor="#66CC66"><div align="center"><b>รายการหนังสือ</b></div></td>
      </tr>
      <tr>
...
End IF
    ...
    'if RS2("barcode") = "9786167441610" Then
...
%>
      ...
      %>
      <tr>
        <td height="25" colspan="5" bgcolor="#FFFFFF"><div align="center"><font color="red">หมายเหตุ : หลังจากวันที่เจ้าหน้าที่ทำการจัดส่งแล้ว ท่านจะได้รับสินค้าประมาณ 3-7 วันทำการนะคะ </font></div> <br><!--font color="red"><div align="center">สำหรับหนังสือภาพถ่ายฝีพระหัตถ์ฯ ขณะนี้มีคนสั่งเข้ามาเป็นจำนวนมาก <br>ขณะนี้กำลังดำเนินการทยอยจัดส่งค่ะ ขออภัยในความล่าช้าค่ะ </div></font--><% Set RSCust=Nothing %></td>
        </tr>
    </table></td>
  </tr>
</table>
