...
If Action="ขั้นตอนถัดไป>>" Then
	Response.Redirect("13082019_test_delivery.asp")
End if
...
Function check_available_item(barcode)
					...
					if  Rs10("disctype") = "C" and Rs10("distribute") = "2" and Rs10("disctype1") = "1" and Rs10("stock_oh")+Rs10("sb_oh")+Rs10("cb_oh") > 3   then				
						check_available_item = 1
							end if
						else
							check_available_item = 1
...
if cur_check_available_item = 1  then  
				  ...
				  If special_discount = "" Then 
					  ...
					  takentotal=takentotal+Session("taken" & p)
					  end if
