...
RS_ChkKerryExpress.Open " SELECT * FROM KerryExpress Where order_no='" & OrderID & "'", Conn, 1, 3

					If RS_ChkKerryExpress.eof Then
						Set RS_ChkMessenger=Server.CreateObject("ADODB.RecordSet")
						RS_ChkMessenger.Open " SELECT * FROM post_messenger Where OrderID='" & OrderID & "'", Conn, 1, 3
						
							If RS_ChkMessenger.eof Then
								Set RS_ChkPostInformation=Server.CreateObject("ADODB.RecordSet")
								RS_ChkPostInformation.Open " SELECT * FROM PostInformation Where OrderID='" & OrderID & "'", Conn, 1, 3
									
									if RS_ChkPostInformation.eof Then
										Set RS_ChkEbook=Server.CreateObject("ADODB.RecordSet")
										RS_ChkEbook.Open " SELECT * FROM orderdetails Where OrderID='" & OrderID & "' and barcode like '%e'", Conn, 1, 3
